...
AMain::AMain()
{
	...
	// kad se okrece mis (ili strelice levo-desno), ne zelimo da se i karakter okrece, nego da stoji u mestu;
	...
	case EStaminaStatus::ESS_BelowMinimum: // ispod minimuma (zuti) rezim
		...
		if (bShiftKeyDown) // pritisnut Shift
		{
			if (bMovingForward || bMovingRight) // ako se krece
			{
				// smanji staminu
				Stamina -= DeltaStamina;
				// ako se smanjila skroz, postavi je na nulu, promeni rezim stamine u exhausted
...
{
	// ako se skida vise nego sto ostalo healtha, umri odmah;
	if (Amount >= Health)
	{
		Health = 0;
		Die();
...
float AMain::TakeDamage(float DamageAmount, struct FDamageEvent const & DamageEvent, class AController * EventInstigator, AActor * DamageCauser)
{
	...
	if (Health - DamageAmount <= 0.f)
	{
		...
		// da bismo mu rekli da batali s napadima, umrp je Main vec
		if (DamageCauser)
		{
